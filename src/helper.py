...
def merge(left_lst, right_lst, final_lst, window, clock, relationstart, relationend):
    ...
    while left_cursor < len(left_lst) and right_cursor < len(right_lst):
        draw_sort_state(window, green=[left_cursor + relationstart], red=[right_cursor + relationstart], blue=[i + relationstart], update=True, clock=clock, fps=fps)
...
def draw_sort_state(window, green=[], red=[], blue=[], done=False, animate=False, clock=None, update=False, fps=0):
    ...
    for i in range(len(lst)):
        ...
        if blue:
            if i in blue: block_surf.fill("#08d2f7")
        if done:
